#include "CameraPrefab_GameSceneMain.h"
...
#include "Game/Definitions/Components/CameraComponents/FreeLookCameraController.h"

namespace Game {

    Engine::GameObject* CameraPrefab_GameSceneMain::Spawn(Engine::SceneContext& ctx, const SpawnDesc& desc) {
        ...
        // カメラ用GameObjectを生成
        auto* obj = ctx.m_scene->CreateObject<Engine::GameObject>("MainCamera");
        if (!obj) {
		...
		obj->AddComponent<FreeLookCameraController>(0.1f);    // マウスで自由に見回せるコントローラー

        return obj;
    }

    Engine::CameraComponent* CameraPrefab_GameSceneMain::GetCameraComponent(Engine::GameObject* cameraObject) {
