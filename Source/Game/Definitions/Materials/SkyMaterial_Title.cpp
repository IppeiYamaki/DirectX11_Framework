...
namespace Game {
    ...
    std::shared_ptr<Engine::Material> SkyMaterial_Title::Create(const Engine::MaterialBuildContext& ctx) {
        ...
        auto tex = ctx.m_assets->LoadTexture(L"Textures/DefoTexture/Texture_Black.png", opt);
        if (tex && tex->IsLoaded()) {
            mat->SetTexture(tex);
