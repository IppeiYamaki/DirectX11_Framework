...
namespace Engine {
    ...
    Input::MessageResult Input::HandleMessage(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
        ...
        case WM_SYSKEYUP:
            ...
            // フォーカスを失ったら押しっぱなしをリセット（Alt+Tab など）
        case WM_KILLFOCUS:
            ClearAllStates();
            return { false, 0 }; // Window 側の既定処理に任せてもOK
        ...
        case WM_XBUTTONDOWN: {
            ...
            return { true, TRUE }; // WM_XBUTTON* は TRUE を返すのが推奨
        }
        case WM_XBUTTONUP: {
            const WORD xbtn = HIWORD(wParam);
